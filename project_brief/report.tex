...
\begin{itemize}
    ...
    \item AI 对话 + 自动闲聊 + 底部输入栏（DeepSeek Function Calling，24+ 工具）
...
AI 模块通过 \texttt{ToolCallInvoker} 注册了 4 个数据查询术式：
...
\begin{tabular}{lll}
\toprule
\textbf{术式名称} & \textbf{API 端点} & \textbf{用途} \\
\midrule
\texttt{query\_scores} & \texttt{/api/pet/scores} & 成绩查询 \\
\texttt{query\_schedule} & \texttt{/api/pet/schedule} & 课表查询 \\
\texttt{query\_exams} & \texttt{/api/pet/exams} & 考试查询 \\
\texttt{query\_user\_status} & \texttt{/api/pet/user/status} & 学业概览 \\
\bottomrule
\end{tabular}
\end{table}
...
+-- code/desktop unity/
    ...
    |   |   +-- ChatManager.cs             # AI 对话管理 + Function Calling
    |   |   +-- AutoChat.cs                # 自动闲聊
    ...
    |   |   +-- ToolCallInvoker.cs         # AI 工具调用（24+ 工具）
